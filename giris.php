...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($mode === 'login') {
        ...
        $find = $pdo->prepare('SELECT * FROM pelish_customers WHERE (email = ? OR username = ?) AND is_active = 1 LIMIT 1');
    ...
    if ($mode === 'register') {
        ...
        if (!preg_match('/^[a-zA-Z0-9._-]{3,80}$/', $data['username'])) {
            store_flash('danger', 'Kullanıcı adı 3–80 karakter olmalı; yalnızca harf, rakam, nokta, alt çizgi ve tire kullanılabilir.');
        } elseif ($data['first_name'] === '' || $data['last_name'] === '' || !filter_var($data['email'], FILTER_VALIDATE_EMAIL) || $data['phone'] === '') {
            ...
            $exists = $pdo->prepare('SELECT COUNT(*) FROM pelish_customers WHERE email = ? OR username = ?');
            $exists->execute([$data['email'], $data['username']]);
            if ((int) $exists->fetchColumn() > 0) {
                store_flash('danger', 'Bu e-posta veya kullanıcı adı zaten kullanılıyor.');
            } else {
                ...
                if (!store_send_verification_email((array) ($config['mail'] ?? []), $data['email'], $code)) {
                    ...
                    $upsert = $pdo->prepare('INSERT INTO pelish_email_verifications (username, first_name, last_name, email, phone, password_hash, code_hash, expires_at, attempts) VALUES (?, ?, ?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL 3 MINUTE), 0) ON DUPLICATE KEY UPDATE username=VALUES(username), first_name=VALUES(first_name), last_name=VALUES(last_name), phone=VALUES(phone), password_hash=VALUES(password_hash), code_hash=VALUES(code_hash), expires_at=DATE_ADD(NOW(), INTERVAL 3 MINUTE), attempts=0');
                    $upsert->execute([$data['username'], $data['first_name'], $data['last_name'], $data['email'], $data['phone'], password_hash($data['password'], PASSWORD_DEFAULT), password_hash($code, PASSWORD_DEFAULT)]);
