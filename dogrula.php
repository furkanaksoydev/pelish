...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!password_verify($code, $verification['code_hash'])) { $pdo->prepare('UPDATE pelish_email_verifications SET attempts=attempts+1 WHERE id=?')->execute([$verification['id']]); store_flash('danger', 'Kod doğru değil. Tekrar deneyebilirsin.'); }
    else {
        try {
...
?>
<main class="store-main auth-main"><section class="verify-card"><p class="eyebrow">SON ADIM</p><h1>E-postanı doğrula.</h1><p><strong><?= e($email) ?></strong> adresine 6 haneli bir kod gönderdik. Kod 3 dakika boyunca geçerli.</p><form method="post" class="auth-form"><input type="hidden" name="csrf" value="<?= e(store_csrf()) ?>"><input type="hidden" name="action" value="verify"><label>Doğrulama kodu<input name="code" required inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" placeholder="000000"></label><button class="button button-dark" type="submit">Hesabımı doğrula <span>→</span></button></form><form method="post" class="resend-form"><input type="hidden" name="csrf" value="<?= e(store_csrf()) ?>"><input type="hidden" name="action" value="resend"><button type="submit">Yeni kod gönder</button></form></section></main>
