...
?>
<main class="store-main"><section class="page-intro compact"><p class="eyebrow">FAVORİLERİM</p><h1>Sevdiklerin.</h1></section><?php if (!$products): ?><section class="commerce-empty full-empty"><p class="eyebrow">PELISH</p><h1>Henüz favori ürünün yok.</h1><p>İyi hissettiren bir parçayı seçtiğinde burada seni bekliyor olacak.</p><a class="button button-dark" href="indirimler.php">İndirimleri keşfet <span>→</span></a></section><?php else: ?><section class="product-grid favorite-product-grid"><?php foreach ($products as $product): ?><article class="product-card db-product-card"><a class="product-image" href="urun.php?id=<?= (int) $product['id'] ?>"><img src="<?= e($product['image_url'] ?: 'https://images.unsplash.com/photo-1485968579580-b6d095142e6e?auto=format&fit=crop&w=900&q=85') ?>" alt="<?= e($product['name']) ?>"></a><div class="product-actions"><form method="post" action="store/action.php"><input type="hidden" name="csrf" value="<?= e(store_csrf()) ?>"><input type="hidden" name="action" value="favorite-remove"><input type="hidden" name="product_id" value="<?= (int) $product['id'] ?>"><input type="hidden" name="return_to" value="favorilerim.php"><button class="heart liked" type="submit" aria-label="Favoriden çıkar"><span>♥</span></button></form><a class="quick-view" href="urun.php?id=<?= (int) $product['id'] ?>">İncele</a></div><a class="product-info" href="urun.php?id=<?= (int) $product['id'] ?>"><div><small><?= e($product['category']) ?></small><h3><?= e($product['name']) ?></h3></div><strong><?= store_money((float) $product['sale_price']) ?></strong></a></article><?php endforeach; ?></section><?php endif; ?></main>
