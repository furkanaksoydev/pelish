...
export default function Home() {
  ...
  return <main>
    ...
    <section className="editorial" id="story"><div className="editorial__image reveal"><img src="https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=1200&q=85" alt="pelish editoryal koleksiyon"/></div><div className="editorial__copy reveal"><p className="eyebrow">PELISH JOURNAL</p><h2>İyi bir parça,<br/>iyi bir <i>hikâye</i> taşır.</h2><p>Atölyeden şehre, tasarımın arkasındaki küçük ama anlamlı ayrıntıları keşfet.</p><a className="text-link" href="#top">Journal’a git {Icons.arrow}</a></div></section>
    <section className="newsletter section reveal"><p className="eyebrow">PELISH’E YAKIN OL</p><h2>Güzel haberler<br/>posta kutunda.</h2><form onSubmit={(e) => { e.preventDefault(); setToast("Teşekkürler, listeye eklendin."); }}><input aria-label="E-posta adresiniz" required type="email" placeholder="E-posta adresin"/><button aria-label="Abone ol">{Icons.arrow}</button></form><small>Abone olarak gizlilik politikamızı kabul etmiş olursun.</small></section>
    <footer><div className="footer-top"><img src="https://cdn.lavira360.com/pelish/logo.png" alt="pelish"/><p>Modern romantiklerin<br/>günlük üniforması.</p><div className="socials"><a href="#top">Instagram</a><a href="#top">Pinterest</a><a href="#top">TikTok</a></div></div><div className="footer-bottom"><span>© 2026 PELISH</span><span>İstanbul, Türkiye</span><span>KVKK · Mesafeli Satış</span></div></footer>

    {welcomeOpen && <div className="overlay welcome"><div className="welcome__card"><button className="modal-close" onClick={() => setWelcomeOpen(false)} aria-label="Kapat">{Icons.close}</button><div className="welcome__art"><span>✦</span><span>✦</span><span>✦</span></div><p className="eyebrow">YENİ BİR BAŞLANGIÇ</p><h2>Yeni adresimize<br/><i>hoş geldin.</i></h2><p>Yeni dünyamızı keşfederken ilk alışverişine özel <b>PELISH10</b> kodu ile %10 indirim seninle.</p><button className="button button--dark" onClick={() => setWelcomeOpen(false)}>Keşfe başla {Icons.arrow}</button></div></div>}
    {menuOpen && <div className="menu-layer"><button className="modal-close menu-close" onClick={() => setMenuOpen(false)} aria-label="Menüyü kapat">{Icons.close}</button><div className="menu-layer__brand">pelish<sup>®</sup></div><nav>{["Yeni Sezon", "Elbiseler", "Üst Giyim", "Alt Giyim", "Dış Giyim", "Aksesuar"].map((item, i) => <a key={item} href="#new" onClick={() => setMenuOpen(false)}><span>0{i + 1}</span>{item}<b>{Icons.arrow}</b></a>)}</nav><div className="menu-layer__bottom"><p>Yeni sezondan haberdar ol.</p><a href="#top">@pelish.official</a></div></div>}
