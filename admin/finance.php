...
{
    if (!in_array($action, ['save-finance-purchase', 'save-finance-payment'], true)) {
        return false;
    }
    try {
        ...
        if ($action === 'save-finance-purchase') {
            $product = form_value('product_name');
            $quantity = max(0, (int) ($_POST['quantity'] ?? 0));
            $unitPrice = max(0, form_number('unit_price'));
            $total = max(0, form_number('total_amount'));
            if ($product === '' || $quantity < 1 || $unitPrice <= 0 || $total <= 0) {
                throw new RuntimeException('Firma, ürün, adet, fiyat ve toplam tutar zorunludur.');
            }
            $statement = $pdo->prepare('INSERT INTO pelish_finance_purchases (supplier_name, product_name, quantity, unit_price, total_amount, invoice_date, note) VALUES (?, ?, ?, ?, ?, ?, ?)');
            $statement->execute([$supplier, $product, $quantity, $unitPrice, $total, admin_finance_date(form_value('invoice_date')), form_value('note') ?: null]);
            admin_flash('success', 'Toptancı faturası kaydedildi.');
        } else {
    ...
    <section class="finance-layout">
      <article class="table-card finance-form-card" id="financePurchase">
        ...
        <form method="post" class="finance-form" data-finance-purchase>
          ...
          <label>Firma adı<input required name="supplier_name" list="financeSuppliers" value="<?= e($supplier) ?>" placeholder="Örn. X Toptan"></label>
          <label>Ürün adı<input required name="product_name" placeholder="Örn. Keten gömlek"></label>
          <label>Kaç adet aldığı<input required type="number" min="1" step="1" name="quantity" value="1" data-finance-quantity></label>
          <label>Birim fiyat<input required type="number" min="0.01" step="0.01" name="unit_price" placeholder="0,00" data-finance-unit-price></label>
          <label class="finance-total">Toplam fiyat<input required type="number" min="0.01" step="0.01" name="total_amount" placeholder="Adet × fiyat" data-finance-total><small>Adet × fiyat otomatik gelir; istersen indirim için elle değiştirebilirsin.</small></label>
